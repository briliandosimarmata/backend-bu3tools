...
public class MstMenuStructureService {
	...
	public byte[] createMenuInfoTSFile(List<MenuStructureDto> menuStructureDtos, String sessionId) throws Exception {
		...
		for (MenuStructureDto menuStructureDto : menuStructureDtos) {
			if (menuStructureDto.getMenuId().trim().isBlank() && menuStructureDto.getMenuSequence().trim().isBlank()) {
				continue;
			}
			SgTempMenuStructureSettingsId menuStructureSettingsId = new SgTempMenuStructureSettingsId(
					menuStructureDto.getMenuId(), menuStructureDto.getMenuSequence(), sessionId);
	...
	@Transactional
	public Map<String, List<MenuStructureDto>> findAllParentAndChildMenuWithSettings(
			List<MenuStructureDto> menuStructureDtos, String sessionId) {
